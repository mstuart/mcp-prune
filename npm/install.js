...
async function main() {
  ...
  const outDir = path.join(import.meta.dirname, "bin");
  const outPath = path.join(
    outDir,
    process.platform === "win32" ? "mcp-prune-bin.exe" : "mcp-prune-bin"
  );
  ...
  await pipeline(
    res,
    zlib.createGunzip(),
    fs.createWriteStream(outPath, { mode: 0o755 })
  );
...
main().catch((err) => {
  ...
  console.error(
    "mcp-prune: the binary will be downloaded on first run, or you can re-run `npm rebuild mcp-prune`."
  );
  process.exit(0);
});
